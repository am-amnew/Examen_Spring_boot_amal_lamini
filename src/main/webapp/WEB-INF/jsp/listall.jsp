...
<%@ page import="com.example.demo.model.HabbitHamster" %>
...
<div class="wrapper" style="margin:60px">
	...
	<h2><strong>All Cats <span>( waiting for adoption )</span> (-_-)</strong></h2>
	...
	<div class="cards">
	 <% List<Cat> catList = (List<Cat>)request.getAttribute("catList");
		...
		<figure class="card" >
			...
			<img src="<%= catList.get(c).getImages() %>" />

			<figcaption>Nom: <%= catList.get(c).getName() %>
			
			 <br>
			 Age: <%= catList.get(c).getAge() %>
			 
			 <br>
       			 Gender: <%= catList.get(c).getGender() %>
       			 
			
       			 
			 <br>
       			 Description: <%= catList.get(c).getDescription() %>
       			 
			 <br>
         <% if (adoptionRepository.findByCatId(catList.get(c).getId()).isPresent()) { %>
                <span style="color: red; font-weight: bold;">Adopted</span>
            <% } else { %>
			
        <form  action="${pageContext.request.contextPath}/adoptions/adoptFormcat/<%=catList.get(c).getId()%>" method="get">
    <button class="text-small-uppercase custom-button"type="submit">Adopt</button>
</form><% } %>
       </figcaption>
	...
	<h2><strong>All Rbbits and humesters<span>( waiting for adoption )</span> (-_-)</strong></h2>

	<div class="cards">
	 <% List<HabbitHamster> habbitHamsterList = (List<HabbitHamster>) request.getAttribute("habbitHamsterList");
                for (int c = 0; c < habbitHamsterList.size(); c++) { %>
		<figure class="card" >

			<img src="<%= habbitHamsterList.get(c).getImages() %>" />

			<figcaption>Nom: <%= habbitHamsterList.get(c).getName() %>
			
			 <br>
			 Age: <%= habbitHamsterList.get(c).getAge() %>
			 
			 <br>
       			 Gender: <%= habbitHamsterList.get(c).getGender() %>
       			 
			
       			 
			 <br>
       			 Description: <%= habbitHamsterList.get(c).getDescription() %>
       			 
			 <br>
      <% if (adoptionRepository.findByHabbitHamsterId(habbitHamsterList.get(c).getId()).isPresent()) { %>
                <span style="color: red; font-weight: bold;">Adopted</span>
            <% } else { %>
			
       <form  action="${pageContext.request.contextPath}/adoptions/adoptFormHabbitHamster/<%=habbitHamsterList.get(c).getId()%>" method="get">
    <button class="text-small-uppercase custom-button"type="submit">Adopt</button>
</form><% } %>
       </figcaption>
  ...
  <div class="wrapper" style="margin:60px">
	...
	<h2><strong>All Birds <span>( waiting for adoption )</span> (-_-)</strong></h2>
	...
	<div class="cards">
	 ...
	 <% List<Bird> birdList = (List<Bird>)request.getAttribute("birdList");
		...
		<figure class="card" >
			...
			<img src="<%= birdList.get(c).getImages() %>" />

			<figcaption>Nom: <%= birdList.get(c).getName() %>
			
			 <br>
			 Age: <%= birdList.get(c).getAge() %>
			 
			 <br>
       			 Color: <%= birdList.get(c).getColor() %>
       			 
			 <br>
       			 Species: <%= birdList.get(c).getSpecies() %>
       			 
			 <br>
       			 Description: <%= birdList.get(c).getDescription() %>
       			 
			 <br>
       
			 <% if (adoptionRepository.findByBirdId(birdList.get(c).getId()).isPresent()) { %>
                <span style="color: red; font-weight: bold;">Adopted</span>
            <% } else { %>
			
			 <form  action="${pageContext.request.contextPath}/adoptions/adoptFormbird/<%=birdList.get(c).getId()%>" method="get">
    <button class="text-small-uppercase custom-button"type="submit">Adopt</button>
</form>
<% } %>
