        ...
        <table >
            ...
            <tbody>
                ...
                for (int c=0; c<dogList.size(); c++){ %>
                    <tr>
                        <td><%= dogList.get(c).getId() %></td>
                        <td><%= dogList.get(c).getName() %></td>
                        <td><%= dogList.get(c).getAge() %></td>
                         <td><%= dogList.get(c).getGender() %></td>
                        <td><%= dogList.get(c).getDescription() %></td>
                                                <td>                <img style="max-width: 100px; max-height: 100px;" src="<%= dogList.get(c).getImages() %>" />
                                                </td>
                        
                        <td><a class="style3" style="background-color:blue;" href="/editdog/<%= dogList.get(c).getId() %>">Edit</a></td>
                        <td><a class="style3" style="background-color:red;" href="#" onclick="confirmDelete('<%= dogList.get(c).getId() %>')">Delete</a></td>
                    </tr>
                <% } %>
            </tbody>
        </table>
        <a class="text-small-uppercase custom-button" href="/newdog" role="button">Add new one </a>
        
    </div>

    <script>
        function confirmDelete(dogId) {
            var confirmDelete = confirm("Are you sure you want to delete this dog?");
            if (confirmDelete) {
                window.location.href = "/deletedog/" + dogId;
            }
        }
    </script>
</body>
